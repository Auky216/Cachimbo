...
const University = ({ prev, next }) => {
  ...
  return (
    <div className="flex h-full w-full flex-col items-center rounded-[3.5rem] bg-cach-l5/20">
      <div className="flex h-[80%] flex-col items-center">
        <div className="mt-14 flex px-1 text-center text-4xl font-bold text-cach-l4 dark:text-cach-l1">
          Elige tu universidad
        </div>
        <div className="flex h-full items-center justify-center">
          <div className="grid w-[85%] grid-cols-3 gap-8 text-sm">
            {universities.map(uni => (
              <button
                key={uni.sigle}
                sigle={uni.sigle} // custom attribute
                className={`min-h-8 w-full items-center justify-center rounded-xl ${clicked_per_univ[uni.sigle] ? "bg-cach-l3/40" : "bg-cach-l3"}  p-2 text-cach-l1`}
                onClick={handleClick}
              >
